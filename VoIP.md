# [Лабораторная работа №2. Базовая настройка IP-телефонов в среде Cisco Packet Tracer](pkt_voip/Lab2.pkt)
...
# [Лабораторная работа №3. Настройка конфигурации Cisco CallManager Express на маршрутизаторе Cisco 2811](pkt_voip/Lab3.pkt)
...
# [Лабораторная работа №4. Конфигурация VOIP в среде Cisco Packet Tracer](pkt_voip/Lab4.pkt)
...
# [Лабораторная работа №7. Построение сети IP-телефонии между удаленными маршрутизаторами в среде Cisco Packet Tracer](pkt_voip/Lab7.pkt)

Цель работы: изучить построение сети IP-телефонии между удаленнымиm филиалами с помощью маршрутизаторов Cisco 2811 и коммутаторов Cisco 2950Т.
...
# [Лабораторная работа №8. Построение сети IP-телефонии между удаленными маршрутизаторами](pkt_voip/Lab8.pkt)
...
1. Последовательное соединение (serial link) между маршрутизаторами - это точка‑точка‑соединение, обычно используемое для WAN‑подключений. Работа строится так:

- Один маршрутизатор выступает DCE (Data Communications Equipment) и задаёт тактовую частоту передачи данных; другой — DTE (Data Terminal Equipment), получает синхронизацию от DCE.

- Физически соединение реализуется через последовательные интерфейсы (например, Serial0/0/0) с использованием кабелей V.35, RS‑232 и т. д.

- На обоих концах настраиваются IP‑адреса на интерфейсах, включается интерфейс, и маршрутизаторы обмениваются трафиком.

2. Скорости зависят от типа линии и оборудования:

- T1 - 1,544 Мбит/с;

- E1 - 2,048 Мбит/с;

- DS0 (канальный интервал T1) - 64 кбит/с;

- низкоскоростные последовательные линии - от 56 кбит/с до 2 Мбит/с (часто настраивается вручную на оборудовании);

- в Cisco Packet Tracer скорости эмулируются - можно задать любую разумную величину через команду clock rate на DCE‑устройстве.

3. Для VoIP (IP‑телефонии) минимальная скорость зависит от кодека и накладных расходов:

- Кодек G.711 (качество PSTN) - около 80–90 кбит/с на вызов (с учётом заголовков RTP/UDP/IP).

- Кодек G.729 (сжатие) - около 24–32 кбит/с на вызов.

- Рекомендуется резервировать не менее 100 кбит/с на один одновременный голосовой вызов с учётом QoS (приоритезация голосового трафика, минимизация задержек и джиттера). Для нескольких вызовов скорость суммируется.

4. LDP (Label Distribution Protocol) - протокол распределения меток в MPLS:

- Маршрутизаторы с включённым LDP обнаруживают соседей через периодические Hello‑сообщения (многоадресная рассылка).

- Между соседями устанавливается сессия TCP.

- LDP обменивается информацией о привязке меток к FEC (Forwarding Equivalence Class — обычно префиксы IP).

- Каждый маршрутизатор назначает локальную метку для FEC и сообщает её соседу.

- При построении сквозного LSP (Label Switched Path) пакеты коммутируются по меткам, а не по IP, что ускоряет пересылку.

5. Типичная архитектура включает 4 уровня:

- Транспортный уровень - обеспечивает доставку пакетов (IP, QoS, DiffServ, MPLS).

- Уровень управления вызовами - сигнализация для установления/завершения звонков (SIP, H.323, MGCP).

- Уровень услуг - реализует дополнительные сервисы (автосекретарь, конференц‑связь, переадресация).

- Уровень абонентского доступа - конечные устройства (IP‑телефоны, софтфоны, шлюзы FXO/FXS для подключения к PSTN).

6. Два основных способа:

- Через шлюз VoIP (VoIP Gateway): IP‑телефон регистрируется на IP‑АТС (например, Cisco Call Manager), которая через шлюз подключает вызов к аналоговой или цифровой линии PSTN (FXO, E1/PRI).

- Через провайдера IP‑телефонии: IP‑телефон или АТС регистрируются на сервере провайдера, который обеспечивает транзит вызова в PSTN.

В Cisco Packet Tracer это моделируется так: IP‑телефон → коммутатор → маршрутизатор/шлюз → эмуляция PSTN.

7. Перевод звонка (Call Transfer):

- Слепой перевод (Blind Transfer) - вызывающий нажимает Transfer, набирает номер и завершает свой вызов; абоненты соединяются напрямую

- Консультируемый перевод (Consultative Transfer) — перед переводом вызывающий соединяется с целевым абонентом, убеждается, что тот готов принять вызов, затем переводит.

Конференц‑связь (Conference Calling): один участник (или IP‑АТС) создаёт конференцию, приглашает остальных. Голос всех участников микшируется (обычно на сервере/АТС), и каждый слышит всех. В SIP это реализуется через B2BUA (Back‑to‑Back User Agent) или MCU (Multipoint Control Unit).

8. Перехват звонка (Call Pickup) позволяет ответить на вызов, предназначенный другому телефону.

- Локальный перехват - в пределах группы (Hunt Group): пользователь нажимает кнопку Pickup или набирает код (например, *8), и его телефон отвечает на звонок, поступивший на любой номер в группе.

- Перехват по группе - пользователь может указать номер группы (например, *8 200), чтобы перехватить звонок, адресованный группе 200.

Механизм реализуется на уровне IP‑АТС: при сигнале перехвата АТС перенаправляет входящий вызов на телефон инициатора перехвата.
